...
def enviar(nombre):
	...
	while sizepiece>0:
		if not piece:
			...
			break
		s.send_multipart([codigo.encode('utf-8'),mihash.encode('utf-8'),nombre.encode('utf-8'),bytes(piece),str(i).encode('utf-8')]) #envia(codigo,hash,nombrearchivo,parte,numeroparte)
		r=s.recv_string()
		piece = f.read(10485760)   #tamano en bytes 10485760 = 10 mb
		sizepiece=sys.getsizeof(piece)
...
def recibir(codigo,hashserver): #recibe(nombre,parte)
	...
	while True:
		s.send_multipart([codigo.encode('utf-8'),hashserver.encode('utf-8'),str(i).encode('utf-8')])
		m=s.recv_multipart()
		...
		if sizepiece<10485760: 
			...
			print('parte recibida del servidor = '+str(i)) #muestra el numero de partes recibidas
